...
public class VentaServiceImpl implements IVentaService {
    ...
    public VentaResponseDTO registrarVenta(VentaRequestDTO dto) {
        ...
        // Validar regla de SUNAT (Ventas mayores a S/ 700.00 requieren identificar al
        // cliente, no se permite Cliente Ocasional / ID 8)
        ...
        if (totalVenta.compareTo(new java.math.BigDecimal("700")) > 0) {
            if (cliente.getIdcliente() == 8 ||
                ...
                throw new IllegalArgumentException(
                        "Por regulación de la SUNAT, las ventas que superan los S/ 700.00 requieren obligatoria e inexcusablemente la identificación del cliente (no se permite Cliente Ocasional).");
            }
        }
        ...
        if (metodoPago.getIdmetodo() != 1 && !metodoPago.getNombre().toLowerCase().contains("efectivo")) {
            ...
            if (metodoPago.getIdmetodo() == 2 || (nameLower.contains("yape") && nameLower.contains("plin"))
                    || nameLower.contains("presencial")) {
                if (!codigo.matches("^\\d{3}$")) {
                    throw new IllegalArgumentException(
                            "El Código de Verificación Dinámico debe ser un número de exactamente 3 dígitos.");
                }
            } else if (metodoPago.getIdmetodo() == 5 || nameLower.contains("e-commerce")
                    || nameLower.contains("pasarela")) {
                if (!codigo.matches("^\\d{6}$")) {
                    throw new IllegalArgumentException(
                            "El Código de Aprobación de Compra debe ser un número de exactamente 6 dígitos.");
                }
            } else if (metodoPago.getIdmetodo() == 4 || nameLower.contains("tarjeta") || nameLower.contains("pos")
                    || nameLower.contains("niubiz") || nameLower.contains("izipay")) {
                if (!codigo.matches("^[a-zA-Z0-9]{4,8}$")) {
                    throw new IllegalArgumentException(
                            "El Número de Operación / Autorización debe ser alfanumérico y tener entre 4 y 8 caracteres.");
                }
            } else if (metodoPago.getIdmetodo() == 3 || nameLower.contains("transferencia")
                    || nameLower.contains("depósito") || nameLower.contains("deposito")) {
        ...
        // Validación de formato de Serie (exactamente 4 caracteres, empezando con B o
        // F)
        if (prefix.length() != 4) {
        ...
        venta.setApplyIgv(dto.getApplyIgv() != null ? dto.getApplyIgv() : true);
        ...
        for (DetalleVentaRequestDTO detalleDTO : dto.getDetalles()) {
            ...
            if (producto.getEstado() == null || !producto.getEstado()) {
                throw new IllegalArgumentException(
                        "El producto [" + producto.getNombre() + "] está inactivo/desactivado.");
            }
            ...
            Inventario inventario = inventarioRepository.findByProducto_Idproducto(producto.getIdproducto())
                    .orElseThrow(() -> new IllegalArgumentException(
                            "No se encontró registro de inventario para el producto: " + producto.getNombre()));

            if (inventario.getEstado() != null && !inventario.getEstado()) {
                throw new IllegalArgumentException(
                        "El inventario del producto [" + producto.getNombre() + "] está inactivo/desactivado.");
            }
            ...
            if (inventario.getStockactual() == null || inventario.getStockactual() < detalleDTO.getCantidad()) {
                ...
                throw new IllegalArgumentException("Stock insuficiente para el producto [" + producto.getNombre()
                        + "]. Stock disponible: " + disponible + ", solicitado: " + detalleDTO.getCantidad());
            }
    ...
    public void anularVenta(Integer id) {
        ...
        for (DetalleVenta d : detalles) {
            Inventario inventario = inventarioRepository.findByProducto_Idproducto(d.getProducto().getIdproducto())
                    .orElseThrow(() -> new IllegalArgumentException(
                            "No se encontró inventario para el producto: " + d.getProducto().getNombre()));
    ...
    private VentaResponseDTO convertirADTO(Venta venta) {
        ...
        dto.setClienteId(venta.getCliente() != null ? venta.getCliente().getIdcliente() : null);
        dto.setClienteTelefono(venta.getCliente() != null ? venta.getCliente().getTelefono() : null);
        dto.setNroPedido(venta.getNroPedido());
        dto.setDireccion(venta.getDireccion());
        dto.setNroOperacion(venta.getNroOperacion());
        dto.setApplyIgv(venta.getApplyIgv() != null ? venta.getApplyIgv() : true);
        ...
        dto.setTipoComprobanteDescripcion(
                venta.getTipoComprobante() != null ? venta.getTipoComprobante().getNombre() : null);
